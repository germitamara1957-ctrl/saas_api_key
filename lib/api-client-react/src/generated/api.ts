...
export const generateVideo = async (
  ...
  return customFetch<VideoJobResponse>(getGenerateVideoUrl(), {
    ...options,
    method: "POST",
    headers: { "Content-Type": "application/json", ...options?.headers },
...
export const getGetVideoStatusQueryKey = (jobId: string) => {
  return [`/api/v1/video/${jobId}`] as const;
};
...
export const testProvider = async (
  ...
  return customFetch<TestProviderResult>(`/api/admin/providers/${id}/test`, {
    method: "POST",
    ...options,
  });
};
...
> => {
  const { mutation: mutationOptions, request: requestOptions } = options ?? {};
  const mutationKey = mutationOptions?.mutationKey ?? ["testProvider"];
  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof testProvider>>,
    { id: number }
  > = ({ id }) => testProvider(id, requestOptions);
...
>(options?: {
  ...
  TContext
> => {
  const mutationOptions = getTestProviderMutationOptions(options);
  return useMutation(mutationOptions);
};

// ─── Portal: Create API Key ───────────────────────────────────────────────────
...
export const createPortalApiKey = async (
  body: CreatePortalApiKeyBody,
  options?: SecondParameter<typeof customFetch>,
): Promise<CreatePortalApiKeyResult> => {
  return customFetch<CreatePortalApiKeyResult>("/api/portal/api-keys", {
    method: "POST",
...
> => {
  const { mutation: mutationOptions, request: requestOptions } = options ?? {};
  const mutationKey = mutationOptions?.mutationKey ?? ["createPortalApiKey"];
  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof createPortalApiKey>>,
    { data: CreatePortalApiKeyBody }
  > = ({ data }) => createPortalApiKey(data, requestOptions);
  return { mutationKey, mutationFn, ...mutationOptions };
};
...
>(options?: {
  ...
  TContext
> => {
  const mutationOptions = getCreatePortalApiKeyMutationOptions(options);
  return useMutation(mutationOptions);
};
