    ...
    <div className="space-y-6">
      ...
      <Dialog open={addCreditsOpen} onOpenChange={setAddCreditsOpen}>
        <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
            <DialogTitle>{t("admin.credits.addCredits")}</DialogTitle>
            <DialogDescription>{t("admin.credits.addCreditsDesc")}</DialogDescription>
          </DialogHeader>
          <Form {...addCreditsForm}>
            <form onSubmit={addCreditsForm.handleSubmit(onAddCreditsSubmit)} className="space-y-4 py-2">
              <p className="text-sm text-muted-foreground">
                Credits will be added to <span className="font-medium text-foreground">{user.name || user.email}</span>'s account balance.
              </p>
              <FormField control={addCreditsForm.control} name="amount" render={({ field }) => (
                <FormItem>
                  <Label>{t("admin.credits.amount")}</Label>
                  <FormControl>
                    <Input type="number" min={1} step={1} {...field} data-testid="input-credits-amount" />
                  </FormControl>
                  <p className="text-xs text-muted-foreground">{t("admin.credits.amountHelp")}</p>
                  <FormMessage />
                </FormItem>
              )} />
              <DialogFooter className="mt-2">
                ...
                <Button type="submit" disabled={isAddCreditsPending} data-testid="button-confirm-credits">
                  {isAddCreditsPending ? "Adding..." : t("admin.credits.addCredits")}
                </Button>
              </DialogFooter>
            </form>
          </Form>
