...
router.post("/v1/responses", requireApiKey, async (req, res): Promise<void> => {
  ...
  if (allowed.length > 0 && !allowed.includes(planModel)) {
    const errMsg = `Model "${model}" is not allowed on your current plan. Allowed: ${allowed.join(", ")}`;
  ...
  if (apiKey.accountCreditBalance < minCost) {
    const errMsg = `Insufficient credits. Balance: $${apiKey.accountCreditBalance.toFixed(6)}.`;
    await db.insert(usageLogsTable).values({
      apiKeyId: apiKey.id, model, inputTokens: 0, outputTokens: 0,
      totalTokens: 0, costUsd: 0, requestId, status: "rejected", errorMessage: errMsg,
    });
